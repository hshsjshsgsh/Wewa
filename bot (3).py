...
CHECK_EMOJI = "<:yes:1464285055193976835>"
...
class Tournament:
    def __init__(self, guild_id, message_id=None):
        ...
        self.mode = "1v1"
        self.match_winners = {}
        self.matched_players_this_round = set()
...
# REMOVED: ShopConfirmView, ShopView for !shop and gem purchase/confirmation.
...
def get_player_display_name(player, guild_id=None, bold=True):
    ...
    if guild_id:
        ...
        # New: add per-user bracket emoji(s) if any
        if hasattr(player, 'id'):
            user_emoji_key = f"user_{player.id}"
            if user_emoji_key in bracket_roles[guild_str]:
                # List or single emoji support
                user_emojis = bracket_roles[guild_str][user_emoji_key]
                if isinstance(user_emojis, list):
                    emojis += user_emojis
                else:
...
class WinnersView(discord.ui.View):
    ...
    async def show_winners(self, interaction: discord.Interaction, button: discord.ui.Button):
        tournament = get_tournament(self.guild_id, self.message_id)
...
class TournamentView(discord.ui.View):
    ...
    async def update_tournament_embed(self, interaction, tournament):
        try:
            ...
            embed.set_image(url="https://cdn.discordapp.com/attachments/1462891303384912078/1464303894266908856/image.png?ex=6974fac5&is=6973a945&hm=4a5a42baf434c57cb5ce7e457330f4c4087197b0bbffa50a825b304e2daa4783")
...
# REMOVED: !add_dash and !remove_dash

# REMOVED: !shop command

@bot.command()
async def signed(ctx):
    """Show only players/teams signed in the current active tournament"""
    signed_text = ""
    found = False
    
    tournament = get_tournament(ctx.guild.id)
    
    if tournament and tournament.max_players > 0 and not tournament.active:
        found = True
        current_count = len(tournament.players) if tournament.mode == "1v1" else len(tournament.players) // 2
        signed_text += f"**Tournament:  {tournament.title}** ({current_count}/{tournament.max_players})\n"
        if tournament.mode == "1v1":
            ...
            for i in range(0, len(tournament.players), 2):
                if i + 1 < len(tournament.players):
                    signed_text += f"- {tournament. players[i].name} & {tournament.players[i+1]. name}\n"
        signed_text += "\n"
    
    if not found:
        await ctx.send("❌ No active tournament registration currently running.")
    else:
...
@app_commands.describe(
    ...
    channel="Channel to post tournament",
...
# --------- NEW COMMANDS FOR USER EMOJIS IN BRACKETS ---------

@bot.command()
async def addemoji(ctx, member: discord.Member, emoji: str):
    """Add an emoji to appear next to the user in brackets."""
    if not has_permission_level_2(ctx.author):
        return await ctx.send("❌ You don't have permission.", delete_after=5)
    guild_str = str(ctx.guild.id)
    user_emoji_key = f"user_{member.id}"
    if guild_str not in bracket_roles:
        bracket_roles[guild_str] = {}

    if user_emoji_key not in bracket_roles[guild_str]:
        bracket_roles[guild_str][user_emoji_key] = [emoji]
    else:
        if isinstance(bracket_roles[guild_str][user_emoji_key], list):
            if emoji not in bracket_roles[guild_str][user_emoji_key]:
                bracket_roles[guild_str][user_emoji_key].append(emoji)
        else:
            # In case a single emoji string was saved, convert to list
            current = bracket_roles[guild_str][user_emoji_key]
            if emoji != current:
                bracket_roles[guild_str][user_emoji_key] = [current, emoji]
    save_data()
    await ctx.send(f"{CHECK_EMOJI} Added emoji {emoji} to {member.display_name} in brackets!", delete_after=5)

@bot.command()
async def removeemoji(ctx, member: discord.Member, emoji: str):
    """Remove an emoji from appearing next to the user in brackets."""
    if not has_permission_level_2(ctx.author):
        return await ctx.send("❌ You don't have permission.", delete_after=5)
    guild_str = str(ctx.guild.id)
    user_emoji_key = f"user_{member.id}"
    removed = False
    if guild_str in bracket_roles and user_emoji_key in bracket_roles[guild_str]:
        val = bracket_roles[guild_str][user_emoji_key]
        if isinstance(val, list):
            if emoji in val:
                bracket_roles[guild_str][user_emoji_key] = [e for e in val if e != emoji]
                # Clean up if no more emojis
                if not bracket_roles[guild_str][user_emoji_key]:
                    del bracket_roles[guild_str][user_emoji_key]
                save_data()
                removed = True
            ...
            if val == emoji:
                ...
                removed = True
    if removed:
        await ctx.send(f"{CHECK_EMOJI} Removed emoji {emoji} from {member.display_name} in brackets!", delete_after=5)
    else:
        await ctx.send("❌ Could not find that emoji on this user in brackets.", delete_after=5)

# -----------------------------------------------------------------

@bot.command()
async def code1v1(ctx, member1: discord.Member, member2: discord.Member, *, code: str):
    """Send match codes only in the channel where tournament is running"""
    try:
        await ctx.message.delete()
    except:
        pass
    ...
    if not tournament or not tournament. target_channel or tournament.target_channel.id != ctx.channel.id:
        return await ctx.send(f"{CROSS_EMOJI} This command can only be used in the tournament channel.", delete_after=5)
...
@bot.command()
async def start(ctx):
    """Start tournament only in the channel where it was created"""
    try:
        await ctx.message.delete()
    except:
        pass
    ...
    tournament.results = []
    tournament.matched_players_this_round = set()
    ...
    embed.set_image(url="https://cdn.discordapp.com/attachments/1462891303384912078/1464303894266908856/image.png?ex=6974fac5&is=6973a945&hm=4a5a42baf434c57cb5ce7e457330f4c4087197b0bbffa50a825b304e2daa4783")
    winners_view = WinnersView(ctx.guild.id, tournament.message_id)
    tournament.message = await ctx.send(embed=embed, view=winners_view)

@bot.command()
async def cancel(ctx):
    """Cancel the tournament in the current channel"""
    try:
        await ctx.message.delete()
    except:
        pass
    ...
    if not has_permission_level_2(ctx.author):
        return await ctx.send(f"{CROSS_EMOJI} You don't have permission to cancel tournaments.", delete_after=5)
    
    tournament = get_tournament(ctx.guild.id)
    
    if not tournament or not tournament.target_channel or tournament.target_channel.id != ctx.channel.id:
        return await ctx.send(f"{CROSS_EMOJI} No tournament running in this channel.", delete_after=5)
    
    tournament_title = tournament.title
    tournament_id = tournament.message_id
    
    if ctx.guild.id in active_tournament_ids:
        del active_tournament_ids[ctx. guild.id]
    
    if (ctx.guild.id, tournament_id) in tournaments:
        del tournaments[(ctx. guild.id, tournament_id)]
    
    await ctx.send(f"{CHECK_EMOJI} Tournament **{tournament_title}** has been cancelled!", delete_after=10)
    await log_command(ctx.guild. id, ctx.author, "! cancel", f"Tournament cancelled:  {tournament_title}")
...
async def winner(ctx, member: discord.Member):
    """Set match winner only in the channel where tournament is running - prevent double qualification"""
    try:
        await ctx.message.delete()
    except:
        pass
    ...
    if tournament.mode == "2v2":  
        ...
        if member_team_id in tournament.matched_players_this_round:  
            return await ctx.send(f"{CROSS_EMOJI} This team has already qualified in this round!", delete_after=5)
        ...
        for i, match in enumerate(current_round):
            ...
            if member in team_a:
                ...
                match_found = True
                match_index = i
                tournament.matched_players_this_round.add(member_team_id)
                break
        ...
        if member.id in tournament.matched_players_this_round:
            return await ctx.send(f"{CROSS_EMOJI} This player has already qualified in this round!", delete_after=5)
        ...
        for i, match in enumerate(current_round):
            ...
            if member == a or member == b:
                ...
                match_found = True
                match_index = i
                tournament.matched_players_this_round.add(member. id)
                break
...
if __name__ == '__main__':
    ...
    if not TOKEN:
        ...
        TOKEN = 'MTQ2MjAzMjI2ODYyODU5NDY4OA.GUUE2O._HeYzX1N8XIubz3Mw1rmKqVve69zDKJwbae8Mc'
