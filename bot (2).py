...
class TournamentView(discord.ui. View):
    ...
    async def update_tournament_embed(self, interaction, tournament):
        try:
            ...
            embed.set_image(url="https://cdn.discordapp.com/attachments/1462029238528905219/1462109996480200765/Screenshot_20260117-174158-7852.png?ex=6972458b&is=6970f40b&hm=523016c3b534d7c8a8d49a142c0285f26dae9c32a0cf939636768130e11506b5")
...
async def shop(ctx):
    ...
    embed = discord.Embed(
        ...
        description=(
            f"{ctx.author.mention}, welcome to our **Stumble Zone Shop**!  Please click under this message what you would like to buy.  {STAR_ICON}\n\n"
...
async def signed(ctx):
    ...
    # Get the active tournament for this guild
...
async def code1v1(ctx, member1: discord.Member, member2: discord.Member, *, code: str):
    ...
    # Check if tournament exists and the command is run in the correct channel
...
async def start(ctx):
    ...
    # Check if command is run in the correct channel
    if not tournament or not tournament.target_channel or tournament.target_channel.id != ctx.channel.id:
        return await ctx.send(f"{CROSS_EMOJI} This command can only be used in the tournament channel.", delete_after=5)
...
async def cancel(ctx):
    ...
    # Check if tournament exists and is in the correct channel
    if not tournament or not tournament.target_channel or tournament.target_channel.id != ctx.channel.id:
        return await ctx.send(f"{CROSS_EMOJI} No tournament running in this channel.", delete_after=5)
    
    # Store tournament info before resetting
    ...
    tournament.__init__(ctx.guild.id, tournament.message_id)
...
async def winner(ctx, member: discord.Member):
    ...
    # Check if tournament exists and command is in the correct channel
    if not tournament or not tournament.target_channel or tournament.target_channel.id != ctx.channel.id:
        return await ctx.send(f"{CROSS_EMOJI} This command can only be used in the tournament channel.", delete_after=5)
    ...
    if tournament.mode == "2v2": 
        ...
        # Check if any team member has already been qualified this round
        for player in [member_team_id]: 
        ...
        # Check if this player has already been qualified this round
